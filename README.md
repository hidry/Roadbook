...
| **Google-Timeline-Import** ✅ (PROGRESS P22) | mittel | hoch | **Umgesetzt:** `Timeline.json`-Adapter (`src/lib/route-model/timeline.ts`) liefert den echten gefahrenen Track unter den Foto-Stopps; Import gefiltert auf den Reisezeitraum, Roh-Dump wird verworfen. Reichere Datenquelle als EXIF (semantisch erkannte Visits + Bewegungssegmente), stärkt den Kern-USP. ⚠️ **Nur manueller Datei-Import** — seit 2024/2025 on-device, keine API/kein Takeout-Cloud-Export mehr. User exportiert `Timeline.json` aus Google Maps, App liest via File-Picker. EU-Variante teils **CSV** → zweiter Parser. Undokumentiertes Format (Community-Schema/GPSBabel) → Wartungsrisiko. ⛔ **DSGVO:** nur extrahierte Stopps/Route speichern, **niemals** den rohen Timeline-Dump (vollständiges Bewegungsprofil). Über Adapter (s. o.). |
